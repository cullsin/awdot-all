...
const { COMMON, CATEGORIES } = errorCodes;
    ...
    if (req.body) {
        ...
        if (errors.length > 0) {
            return res.status(200).send({
                ...
                message: CATEGORIES[validation.errorCode],
                errors: errors.join(',') 
            });
        } else {
            return next();
...
export function hasValidInsertFields(req, res, next) {
    const validation = {
        fields : ['user_id', 'name', 'companies_id'],
        errorCode: '001'
    }
...
export function hasValidRemoveFields(req, res, next) {
    const validation = {
        fields : ['user_id', 'catcompanies_id'],
        errorCode: '005'
    }
    return _validateFields(validation, req, res, next); 
}
