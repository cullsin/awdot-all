...
const { COMMON, TRANSACTION } = errorCodes;
    ...
    if (req.body) {
        ...
        if (errors.length > 0) {
            return res.status(200).send({
                ...
                message: TRANSACTION[validation.errorCode],
                errors: errors.join(',') 
            });
        } else {
            return next();
...
export function hasValidInsertFields(req, res, next) {
    const validation = {
        fields : ['user_id', 'number_of_shares', 'from_id', 'to_id', 'created_by', 'total_amount', 'share_price'],
        errorCode: '001'
    }
